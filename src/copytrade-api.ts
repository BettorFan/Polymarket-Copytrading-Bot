...
async function main() {
    ...
    if (env.DRY_RUN) logger.warn("⚠️  DRY_RUN=true: Set DRY_RUN=false in .env to place real orders.");
...
================================================================================\n\n`
    ...
    try {
        ...
        logger.warn(`CLOB error (continuing): ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (env.ORDER_SIZE_IN_TOKENS && client) {
        try {
            ...
            logger.warn("Balance check failed – continuing");
        }
    }
    ...
    try {
        ...
        logger.warn("Allowances failed – will retry on first trade\n");
    }

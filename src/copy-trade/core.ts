...
export async function retryWithBackoff<T>(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            logger.warn(`⚠️  ${operationName} failed (attempt ${attempt}/${maxAttempts}): ${fullMsg.substring(0, 80)}`);
...
async function executeBuyFromPending(client: ClobClient, pending: PendingBuy, currentPrice: number): Promise<boolean> {
    ...
    try {
        ...
        logger.warn(`Pending BUY failed ${pending.tokenId.substring(0, 14)}...: ${msg.substring(0, 50)}`);

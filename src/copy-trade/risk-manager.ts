...
async function sellToken(client: ClobClient, tokenId: string, conditionId: string, _limitPrice: number): Promise<boolean> {
    ...
    for (let attempt = 1; attempt <= RISK_SELL_MAX_ATTEMPTS; attempt++) {
        try {
            ...
            logger.warn(`🛡️ Risk sell attempt ${attempt}/${RISK_SELL_MAX_ATTEMPTS}: not filled status=${rawStatus ?? "null"}`);
            logToFile(`RISK SELL RETRY: attempt ${attempt} tokenId ${tokenId.substring(0, 18)}... error=${String(errMsg).slice(0, 80)}`);
        } catch (e) {
            const msg = e instanceof Error ? e.message : String(e);
            logger.warn(`🛡️ Risk sell attempt ${attempt}/${RISK_SELL_MAX_ATTEMPTS} failed: ${msg.substring(0, 60)}`);
    ...
    logger.warn(`Risk sell gave up after ${RISK_SELL_MAX_ATTEMPTS} attempts for ${tokenId.substring(0, 14)}...`);
        ...
        for (const [tokenId, { conditionId, buyPrice }] of monitored.entries()) {
            try {
                ...
                if (price === null) {
                    ...
                    logger.warn(`🛡️ Risk manager: could not get price for tokenId ${tokenId.substring(0, 16)}... (response: ${raw})`);
                ...
                if (price >= env.PROFIT_SELL_THRESHOLD) {
                    logger.warn(`🛡️ Risk manager: price ${price} >= ${env.PROFIT_SELL_THRESHOLD} (profit cap) → selling immediately`);
                    const sold = await sellToken(client, tokenId, conditionId, price);
                    toRemovePrice.push(tokenId);
                } else if (price < env.SELL_PRICE) {
                    logger.warn(`🛡️ Risk manager: price ${price} < ${env.SELL_PRICE} → selling`);
                    const sold = await sellToken(client, tokenId, conditionId, price);
                    toRemovePrice.push(tokenId);
                }
            } catch (e) {
                const msg = e instanceof Error ? e.message : String(e);
                logger.warn(`🛡️ Risk manager: getPrice/sell failed for ${tokenId.substring(0, 16)}...: ${msg.substring(0, 50)}`);
                toRemovePrice.push(tokenId);
            }
        }

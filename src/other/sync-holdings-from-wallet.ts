...
async function getTokensFromWallet(
    ...
    if (knownConditionIds.length > 0) {
        ...
        for (const conditionId of knownConditionIds) {
            try {
                ...
                logger.warn(`   ⚠️  Failed to check conditionId ${conditionId.substring(0, 12)}...`);
                continue;
            }
        }
    }
    ...
    try {
        ...
        logger.warn(`   Error scanning transfer events: ${error instanceof Error ? error.message : String(error)}`);
    }
...
async function main() {
    logger.info("🔄 SYNC HOLDINGS FROM WALLET");
    logger.info("=".repeat(70));

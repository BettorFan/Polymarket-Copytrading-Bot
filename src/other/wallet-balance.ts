...
async function main() {
    ...
    try {
        ...
        logger.warn(`⚠️  Failed to resolve proxy wallet: ${e instanceof Error ? e.message : String(e)}`);
    ...
    try {
        ...
        logger.warn(`⚠️  Could not fetch CLOB balance/allowance: ${e instanceof Error ? e.message : String(e)}`);
    ...
    try {
        ...
        logger.warn(`⚠️  Could not fetch on-chain USDC balance: ${e instanceof Error ? e.message : String(e)}`);
    }
}
